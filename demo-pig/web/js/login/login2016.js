...
var LoginConstant = {
    HTTP_SCHEME: "http:",
    HTTPS_SCHEME: "https:",
    REMOTE_COUNTRY_CODE: "login/login.countrycode-170524.js",
    LOCAL_COUNTRY_CODE: "login/login.countrycode-170524.js"
};
...
(function(){
    ...
    function loginSubmit(callback) {
        ...
        if(window.location.href.indexOf("/popupLogin")!=-1){
        ...
        $.ajax({
            url: loginUrl + "?"+ location.search.substring(1) + "&r=" + Math.random()+"&version=2017",
            type: "POST",
            dataType: "text",
            contentType: "application/x-www-form-urlencoded; charset=utf-8",
            data: {
            ...
            success: function (result) {
                if (result) {
                    ...
                    if (obj.venture) {
                        window.location = "//x/dangerousVerify/index.action?username=" + obj.venture + "&ReturnUrl=" + encodeURI(obj.ventureRet) + "&p=" +obj.p+ "&t=" + new Date().getTime();
                        return;
                    }
                    if (obj.resetpwd) {
                        window.location = "//x/resetPwd/reset.action?username=" + obj.resetpwd;
                        return;
                    }
                    if (obj.rescue) {
            ...
            success: function (result) {
                if (result) {
                    ...
                    if (obj.success || obj.transfer) {
                        Util.Cookie.setALCookie();
                        var relayUrl = protocol + '//x/relay/loginRelay.htm';
                        try{
