...
import com.bmn.core.ApplicationRunner;
...
public class BmnApplication {
    ...
    private static final Logger logger = LogManager.getLogger(BmnApplication.class);

    private Set<Class<?>> primaryClass;

    private BmnApplication(Class<?>... primaryClass) {
    ...
    public static void run(Class<?>[] primaryClass, String[] args) {
        new BmnApplication(primaryClass).run(args);
    }

    public static void main(String[] args) {
        BmnApplication.run(new Class<?>[0], args);
    }
}
